...
exports.generateIshiharaPdf = exports.generateBMIPdf = void 0;
const pdfkit_1 = __importDefault(require("pdfkit"));
const chartjs_node_canvas_1 = require("chartjs-node-canvas");
const stream_1 = require("stream");
const getBufferFromDoc = (doc) => {
...
const width = 400;
const height = 200;
const generateBMIPdf = async (data) => {
    ...
    doc.moveDown(0.5);
    doc.text(`Fogarty Center ideal range for ${data.sex.toLowerCase()} at ${(data.responses.height / 100).toFixed(2)} m: 66–82 kg. Obesity: >= 98 kg.`);
    doc.text(`Healthy weight range (BMI 18.5–24.9): 53.5 – 72.0 kg`);
    doc.moveDown(1.5);
    // ---------------- WHO TABLE ----------------
    ...
    doc.fillColor('black');
    doc.moveDown(0.3);
    doc.fontSize(8).text('Fogarty Center Conference on Obesity (1973, Bray 1979) & Metropolitan Life Insurance Tables (1959)');
    doc.moveDown(0.5);
    const idealRows = [
        ['1.60', '59 – 74', '>= 89'],
        ['1.62', '60 – 75', '>= 90'],
    ...
    doc.moveDown(1);
    // ---------------- DISCLAIMER ----------------
    doc.fontSize(8).fillColor('gray').text('Disclaimer: BMI is a screening tool, not diagnostic. Consult a healthcare professional.');
    doc.moveDown(0.5);
    doc.text('Source: Fogarty Center Conference (1973) & WHO BMI guidelines.');
    doc.moveDown(0.5);
    doc.text('GlobalECare.com — BMI Calculator & Report Tool');
    doc.end();
    return doc;
};
exports.generateBMIPdf = generateBMIPdf;
...
// return doc;
// };
const getInterpretationText = (category) => {
    if (category === 'Normal') {
...
const getHealthyRange = (height) => {
    const h = height / 100;
    const min = (18.5 * h * h).toFixed(1);
    const max = (24.9 * h * h).toFixed(1);
    return `${min} – ${max}`;
};
const getInterpretation = (category) => {
    switch (category) {
        case 'Normal':
...
//   return doc;
// };
const generateIshiharaPdf = (data) => {
    const doc = new pdfkit_1.default({ size: 'A4', margin: 40 });
    const { patient = {}, responses = {}, result = {} } = data;
    ...
    doc
        .fillColor('#0b3d2e')
        .fontSize(16)
        .text(`${correct} / ${total} (${accuracy}%)`, boxX, boxY + padding, {
        width: boxWidth,
        align: 'center'
    });
    ...
    doc
        .fontSize(11)
        .fillColor('#333')
        .text(result.classification || '', boxX, boxY + padding + 22, {
        width: boxWidth,
        align: 'center'
    });
    ...
    doc
        .fontSize(8)
        .fillColor('#666')
        .text('Disclaimer: This is a screening test and not a medical diagnosis.', { align: 'center' });
    doc.end();
    return doc;
};
exports.generateIshiharaPdf = generateIshiharaPdf;
const getResultColor = (vision) => {
    if (vision === 'normal')
        return 'green';
    if (vision === 'mild')
        return 'orange';
